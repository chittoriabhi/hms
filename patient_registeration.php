...
<a href="#mModal" class="trigger-btn btn btn-primary" data-toggle="modal">Click here to Register </a>  
	...
	<div class="modal-dialog modal-login">
		<div class="modal-content">
			<div class="modal-header">				
				<h4 class="modal-title">Patient SignUp</h4>
			...
			<div class="modal-body">
				<form action="user_login.php" method="post">
					...
					<div class="form-group">
						...
						<div class="form-group">
						<i class="fa fa-user"></i>
						<input type="text" class="form-control" placeholder="City" required="required" name="p_city">
					</div>
					<div class="form-group">
								<label class="block">
									Gender
...
if (isset($_POST['submit'])) {
	...
	$cmd=mysqli_query($con,"select p_username from patients where p_username='$p_username'");
	$cmd1=mysqli_num_rows($cmd);
  	
	if($p_c_password==$p_password){
		if($cmd1==0){

		$insert_patient="insert into patients(p_name,p_username,p_address,p_contact,p_city,p_gender,p_email,p_password) values('$p_name','$p_username','$p_address','$p_contact','$p_city','$p_gender','$p_email','$p_password') ";
		$run_patient=mysqli_query($con,$insert_patient);
		$fetch_id=mysqli_query($con,"select p_id from patients where p_email='$p_email' ");
		$fetch=mysqli_fetch_array($fetch_id);
		$pid=$fetch['p_id'];
		$p_image="logo.jpg";
		$inset_image=mysqli_query($con,"insert into patient_images (p_id,p_image) values('$pid','$p_image')");
		echo "<script>alert('Registerd')</script>";}
	}else{
		echo "<script>alert('Password didnt match or username exists')</script>";
		echo "<script>window.open('patient_registeration.php','_self')</script>";
	}
